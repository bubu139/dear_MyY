...
// Lặp qua tất cả các phần tử có class "item"
const items = document.querySelectorAll('.item');
items.forEach(item => {
  // Lấy giá trị data-item-size, ví dụ: "1,2"
  const [x, y] = item.getAttribute('data-item-size').split(',');
  // Gán biến CSS
  item.style.setProperty('--item-size-x', x);
  item.style.setProperty('--item-size-y', y);
});

document.querySelectorAll('.item').forEach(item => {
  const [x, y] = item.getAttribute('data-item').split(',');
...
const existingCoords = new Set([
  "-37,-4", "-37,-2", "-37,0", "-37,2", "-37,4", /* ... other provided coordinates ... */
]);
let output = '';
for (let i = -50; i <= 50; i++) {
  for (let j = -50; j <= 50; j++) {
    ...
    if (!existingCoords.has(coord)) {
      output += `
        <div class="item" data-src="anh/placeholder.jpg" data-item="${i},${j}" data-item-size="2,2">
          <div class="item__image">
            <img src="anh/placeholder.jpg" draggable="false"/>
          </div>
        </div>`;
    }
  }
